...
//		System.out.println(generatePalindromes("abc"));
	...
	public static List<String> generatePalindromes(String s) {
        ...
        List<Character> list = new ArrayList<>();
        Map<Character, Integer> map = new HashMap<>();
        // build character leetcode.sort map and leetcode.sort odds
        for (int i = 0; i < s.length(); i++) {
        	char c = s.charAt(i);
        	map.put(c, map.containsKey(c) ? map.get(c) + 1: 1);
        	odd += map.get(c) % 2 != 0 ? 1: -1;
        }
        // can't form any palindromic leetcode.string
        if (odd > 1) return res;
        // add half leetcode.sort of each character to list
	...
	private static void getPerm(List<Character> list, String mid, boolean[] used,
		...
		if (sb.length() == list.size()) {
			// form the palindromic leetcode.string
